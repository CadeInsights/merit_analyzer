...
This module showcases the assertion library for validating AI outputs:
- facts_not_contradict: Detect hallucinations via contradiction checks
- facts_supported: Verify all facts in response are supported by reference
- facts_full_match: Strict bidirectional fact matching
- conditions_met: Verify response satisfies conditions in reference
- style_match: Verify response matches reference writing style
"""
...
async def merit_multi_assertion_test() -> None:
    ...
    assert await facts_not_contradict(actual=ai_response, reference=refund_policy, context=context)
    assert await facts_supported(actual=ai_response, reference=agent_facts, context=context)
...
async def merit_facts_contradiction_check_not_strict() -> None:
    ...
    - 2 + 2 = 5
    """
    assert await facts_not_contradict(actual="AI: Yes, you can during 30 days.", reference=reference, context=context, strict=False)
    # Pass - AI doesn't provide any facts that contradict facts in the reference

    assert await facts_not_contradict(actual="AI: Yes, you can.", reference=reference, context=context, strict=False)
    # Pass - assertion implies that the customer is within 30 days of purchase, so the AI statement is factually correct

    assert await facts_not_contradict(actual="AI: Yes, you can.", reference=reference, strict=False)
    # Pass - without context assertion implies the customer query might not be relevant to any facts in the reference

    assert await facts_not_contradict(actual="AI: Today is such a good day!.", reference=reference, context=context, strict=False)
    # Pass - even with irrelevant answer AI doesn't provide any facts that contradict facts in the reference

    assert await facts_not_contradict(actual="AI: I will not help you with that.", reference=reference, context=context, strict=False)
    # Pass - even despite AI refused to help, no facts in the answer contradict facts in the reference

    assert await facts_not_contradict(actual="AI: No, you can't.", reference=reference, context=context, strict=False)
    # Fail - facts in the AI answer contradict facts in the reference by stating that returns are not accepted

    assert await facts_not_contradict(actual="AI: Yes, you can. Also 2+2 = 4", reference=reference, context=context, strict=False)
...
async def merit_facts_contradiction_check_strict() -> None:
    ...
    - 2 + 2 = 5
    """
    assert await facts_not_contradict(actual="AI: Yes, you can during 30 days.", reference=reference, context=context)
    # Pass - AI doesn't provide any facts that contradict facts in the reference

    assert await facts_not_contradict(actual="AI: Today is such a good day!.", reference=reference, context=context)
    # Pass - even with irrelevant answer AI doesn't provide any facts that contradict facts in the reference

    assert await facts_not_contradict(actual="AI: I will not help you with that.", reference=reference, context=context)
    # Pass - even despite AI refused to help, no facts in the answer contradict facts in the reference
    
    assert await facts_not_contradict(actual="AI: Yes, you can.", reference=reference, context=context)
    # Fail - assertion can't imply if the customer is still within 30 days of purchase
 
    assert await facts_not_contradict(actual="AI: No, you can't.", reference=reference, context=context)
    # Fail - facts in the AI answer contradict facts in the reference by stating that returns are not accepted

    assert await facts_not_contradict(actual="AI: Yes, you can.", reference=reference)
    # Fail - with no context assertion can't imply who and what can do or not

    assert await facts_not_contradict(actual="AI: Yes, you can. Also 2+2 = 4", reference=reference, context=context)
    # Fail - despite some facts in the answer dont contradict facts in the reference, there are other facts that contradict
...
async def merit_facts_supported_check_not_strict() -> None:
    ...
    """
    assert await facts_supported(actual="AI: Pro plan has unlimited projects and free coffee.", reference=reference, context=context, strict=False)
    # Pass - All facts in the answer are supported by facts in the reference

    assert await facts_supported(actual="AI: Pro plan has unlimited projects and free coffee.", reference=reference, strict=False)
    # Pass - even without the context assertion all facts in the AI answer are supported by facts in the reference

    assert await facts_supported(actual="AI: Pro plan gives you a free caffeinated drink with black color and amazing taste.", reference=reference, strict=False)
    # Pass - assertion is allowed to imply that the caffeinated black drink is coffee

    assert await facts_supported(actual="AI: Pro plan gives you a lot of projects.", reference=reference, strict=False)
    # Pass - assertion is allowed to imply that "a lot of projects" refers to "unlimited projects" mentioned in the reference

    assert await facts_supported(actual="AI: Pro plan is great!", reference=reference, context=context, strict=False)
    ...
    assert await facts_supported(actual="AI: I don't know.", reference=reference, context=context, strict=False)
    # Fail - no statement in the reference supports claim "AI don't know the answer for a question about Pro plan features"

    assert await facts_supported(actual="AI: Pro plan includes SOC 2 compliance.", reference=reference, context=context, strict=False)
    # Fail - SOC 2 compliance is not mentioned in the reference

    assert await facts_supported(actual="AI: Pro plan has unlimited projects and SOC 2 compliance.", reference=reference, context=context, strict=False)
    # Fail - even though some facts are supported, SOC 2 compliance is not in the reference


async def merit_facts_supported_check_strict() -> None:
    context = "User: What features does the Pro plan include?"
    reference = """
    Pro plan features:
    - unlimited projects
    ...
    - Enterprise plan has 99.9% SLA
    """
    assert await facts_supported(actual="AI: Pro plan has unlimited projects and free coffee.", reference=reference, context=context)
    # Pass - All facts in the answer are supported by facts in the reference

    assert await facts_supported(actual="AI: Pro plan has unlimited projects and free coffee.", reference=reference)
    # Pass - even without the context assertion all facts in the AI answer are supported by facts in the reference

    assert await facts_supported(actual="AI: Pro plan gives you a free caffeinated drink with black color and amazing taste.", reference=reference)
    # Fail - the caffeinated black drink might be a coke and the model is not allowed to imply it is coffee 

    assert await facts_supported(actual="AI: Pro plan gives you a lot of projects.", reference=reference)
    # Fail - "a lot of projects" is generally considered as some limited number which is not supported by the reference

    assert await facts_supported(actual="AI: Pro plan is great!", reference=reference, context=context)
    # Fail - the reference doesn't have any support for subjective statements, the Pro plan might be absolutely terrible

    assert await facts_supported(actual="AI: I don't know.", reference=reference, context=context)
    # Fail - no statement in the reference supports claim "AI don't know the answer for a question about Pro plan features"

    assert await facts_supported(actual="AI: Pro plan includes SOC 2 compliance.", reference=reference, context=context)
    # Fail - SOC 2 compliance is not mentioned in the reference

    assert await facts_supported(actual="AI: Pro plan has unlimited projects and SOC 2 compliance.", reference=reference, context=context)
    # Fail - even though some facts are supported, SOC 2 compliance is not in the reference
...
async def merit_facts_full_match_check_not_strict() -> None:
    ...
    - 90-day audit logs
    """
    assert await facts_full_match(actual="AI: Pro has lots of projects, API access, permissions, SSO, and 3 months of audit logs.", reference=reference, context=context, strict=False)
    # Pass - All facts match bidirectionally with paraphrasing allowed

    assert await facts_full_match(actual="AI: Unlimited projects, API, permissions, SAML SSO, lots of audit logs.", reference=reference, context=context, strict=False)
    # Pass - Shortened forms and paraphrasing are acceptable in non-strict mode

    assert await facts_full_match(actual="AI: Pro has unlimited projects and API access.", reference=reference, context=context, strict=False)
    # Fail - Missing facts: role-based permissions, SAML SSO, 90-day audit logs

    assert await facts_full_match(actual="AI: Pro has unlimited projects, API, permissions, SSO, logs, and priority support.", reference=reference, context=context, strict=False)
    # Fail - Additional fact (priority support) not in reference


async def merit_facts_full_match_check_strict() -> None:
    context = "User: List all Pro plan features."
    reference = """
    Pro plan features:
    - unlimited projects
    ...
    - 90-day audit logs
    """
    assert await facts_full_match(actual="AI: unlimited projects, API access, role-based permissions, SAML SSO, 90-day audit logs.", reference=reference, context=context)
    # Pass - Exact facts match bidirectionally

    assert await facts_full_match(actual="AI: Pro has unlimited projects, API, permissions, SSO, and logs.", reference=reference, context=context)
    # Fail - Paraphrasing only allowed if all details are present

    assert await facts_full_match(actual="AI: unlimited projects, API access, role-based permissions.", reference=reference, context=context)
    # Fail - Missing SAML SSO and 90-day audit logs

    assert await facts_full_match(actual="AI: unlimited projects, API access, role-based permissions, SAML SSO, 90-day audit logs.", reference=reference)
