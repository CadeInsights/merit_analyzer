...
| `raise_on_error` | `bool` | `True` | Whether to raise on validation errors |

**Returns:** `Sequence[Case[RefsT]]` - Valid cases (or all cases if validation passed)

**Raises:** `ValidationError` if any case's inputs don't match SUT signature (when `raise_on_error=True`)
...
try:
    valid_cases = valididate_cases_for_sut(cases, my_agent)
except ValidationError as e:
    print(f"Invalid case: {e}")

# Or filter out invalid cases
valid_cases = valididate_cases_for_sut(cases, my_agent, raise_on_error=False)
```
